...
def universal_gradient_descent_method(
    oracle,
    prox,
    primal_dual_oracle,
    t_start,
    ...
    if stop_crit == "dual_gap_rel":
        ...
        raise ValueError(
            "stop_crit should be callable or one of the following names: \
    ...
    primal, dual, duality_gap_init, state_msg = primal_dual_oracle(
        flows_weighted, t_weighted
    )
    if save_history:
    ...
    for it_counter in range(1, max_iter + 1):
        while True:
            ...
            left_value = (
                oracle.func(t_prev) + np.dot(grad_t, t - t_prev) + 0.5 * eps_abs
            ) - oracle.func(t)
        ...
        primal, dual, duality_gap, state_msg = primal_dual_oracle(
            flows_weighted, t_weighted
        )
        if save_history:
